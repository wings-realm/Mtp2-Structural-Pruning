...
commands = [
    {
        ...
        "cmd": f"python3 ddpm_hybrid_hessian.py --dataset cifar10 --model_path pretrained/ddpm_ema_cifar10 --save_path run/pruned/ddpm_cifar10_pruned --pruning_ratio {pruning_ratio} --batch_size 32 --lambda_taylor 0.4 --lambda_latent 0.2 --lambda_hessian 0.4 --device {{device}}"
    },
    
    {
        "desc": "Step 2: Fine-tuning the pruned model",
        "cmd": "python3 ddpm_train.py --dataset=cifar10 --model_path=run/pruned/ddpm_cifar10_pruned --pruned_model_ckpt=run/pruned/ddpm_cifar10_pruned/pruned/unet_pruned.pth --resolution=32 --output_dir=run/finetuned/ddpm_cifar10_pruned_post_training --train_batch_size=128 --num_iters=250000 --gradient_accumulation_steps=1 --learning_rate=2e-4 --lr_warmup_steps=0 --save_model_steps=1000 --dataloader_num_workers=8 --adam_weight_decay=0.00 --ema_max_decay=0.9999 --dropout=0.1 --use_ema --logging_dir=run/logs/ddpm_cifar10_pruned --device {device}"
    },
    {
